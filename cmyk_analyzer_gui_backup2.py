...
def detect_special_color(img: np.ndarray, exclude_colors: dict) -> tuple:
    """
    이미지에서 특별한 색상을 감지합니다.
    왼쪽 상단 영역에서 C, M, Y 색상을 제외한 색상을 찾습니다.
    
    Args:
        img: 입력 이미지 (BGR)
        exclude_colors: 제외할 색상들의 HSV 범위 딕셔너리
    
    Returns:
        tuple: (감지된 색상의 HSV 범위, 색상 이름) 또는 (None, None)
    """
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    h, w = img.shape[:2]
    
    # 왼쪽 상단 영역 정의 (전체 이미지의 1/4 영역)
    left_top_region = hsv[0:h//2, 0:w//2]
    
    # 제외할 색상들의 마스크 생성 (왼쪽 상단 영역에서)
    exclude_mask = np.zeros((h//2, w//2), dtype=np.uint8)
    for color_name, hsv_range in exclude_colors.items():
        mask = cv2.inRange(left_top_region, np.array(hsv_range[0]), np.array(hsv_range[1]))
        exclude_mask = cv2.bitwise_or(exclude_mask, mask)
    
    # 제외할 색상들을 제외한 영역에서 색상 클러스터링
    remaining_pixels = cv2.bitwise_not(exclude_mask)
    
    if np.sum(remaining_pixels) < 500:  # 충분한 픽셀이 없으면 None 반환
        return None, None
    
    # 남은 픽셀들의 HSV 값들 수집
    remaining_hsv = left_top_region[remaining_pixels > 0]
    
    if len(remaining_hsv) < 50:  # 충분한 샘플이 없으면 None 반환
        return None, None
    ...
    try:
        ...
        # HSV 값을 정규화
        hsv_normalized = remaining_hsv.astype(np.float32)
        hsv_normalized[:, 0] = hsv_normalized[:, 0] / 180.0  # H를 0-1로 정규화
        hsv_normalized[:, 1:] = hsv_normalized[:, 1:] / 255.0  # S, V를 0-1로 정규화
        
        # K-means 클러스터링 (2개 클러스터로 줄임)
        kmeans = KMeans(n_clusters=2, random_state=42, n_init=10)
        kmeans.fit(hsv_normalized)
        ...
        # 가장 큰 클러스터의 중심점
        center = kmeans.cluster_centers_[largest_cluster_idx]
        
        # 정규화된 값을 원래 스케일로 변환
        h_center = int(center[0] * 180)
        s_center = int(center[1] * 255)
        v_center = int(center[2] * 255)
        
        # HSV 범위 생성 (중심점 주변, 더 좁은 범위)
        h_tolerance = 15
        s_tolerance = 40
        v_tolerance = 40
        
        hsv_lower = (max(0, h_center - h_tolerance), max(0, s_center - s_tolerance), max(0, v_center - v_tolerance))
        hsv_upper = (min(180, h_center + h_tolerance), min(255, s_center + s_tolerance), min(255, v_center + v_tolerance))
        ...
        # sklearn이 없는 경우 간단한 방법 사용
        # 남은 픽셀들의 평균 HSV 값 사용
        mean_hsv = np.mean(remaining_hsv, axis=0)
        h_center = int(mean_hsv[0])
        s_center = int(mean_hsv[1])
        v_center = int(mean_hsv[2])
        ...
        return (hsv_lower, hsv_upper), "Special_Color"
...
class AnalysisWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # 전체 색상 범위 (special color 포함)
            HSV = base_colors.copy()
            HSV['S'] = special_hsv_range  # Special color를 'S'로 표시
            ...
            target_coords = {
                'S': (w_px/10, h_px - h_px*6/10),  # Special color가 K 위치에
...
💡 TIP: Make small adjustments and re-analyze for best results.
    ...
    def populate_overview_cards(self, results):
        ...
        # Special color 이름을 동적으로 가져오기 위해 results에서 추출
