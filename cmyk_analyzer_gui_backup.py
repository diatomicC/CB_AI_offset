...
def detect_special_color(img: np.ndarray, exclude_colors: dict) -> tuple:
    """
    이미지에서 특별한 색상을 감지합니다.
    exclude_colors에 정의된 색상들을 제외하고 가장 큰 영역을 차지하는 색상을 찾습니다.
    ...
    if np.sum(remaining_pixels) < 1000:  # 충분한 픽셀이 없으면 None 반환
        return None, None
    
    # 남은 픽셀들의 HSV 값들 수집
    remaining_hsv = hsv[remaining_pixels > 0]
    
    if len(remaining_hsv) < 100:  # 충분한 샘플이 없으면 None 반환
        return None, None
